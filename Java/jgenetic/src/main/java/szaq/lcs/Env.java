...
public class Env {
	...
	public Env(final int size) {
		...
		IntStream.range(0, elementsCount).forEach(x->trainingSet.add(new Rule(getId(), WildcardChromosome.randomFalse(), 0,0,0,0)));
		IntStream.range(0, elementsCount).forEach(x->trainingSet.add(new Rule(getId().toString(), WildcardChromosome.randomTrue(), 1,0,0,0)));
